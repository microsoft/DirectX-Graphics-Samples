...
namespace Math
{
	...
	public:
		INLINE Matrix3() {}
		INLINE Matrix3( const Vector3 &x, const Vector3 &y, const Vector3 &z ) { m_mat[0] = x; m_mat[1] = y; m_mat[2] = z; }
		INLINE Matrix3( const Matrix3& m ) { m_mat[0] = m.m_mat[0]; m_mat[1] = m.m_mat[1]; m_mat[2] = m.m_mat[2]; }
		INLINE Matrix3( const Quaternion &q ) { *this = Matrix3(XMMatrixRotationQuaternion(q)); }
		...
		INLINE void SetX(const Vector3 &x) { m_mat[0] = x; }
		INLINE void SetY(const Vector3 &y) { m_mat[1] = y; }
		INLINE void SetZ(const Vector3 &z) { m_mat[2] = z; }
		...
		static INLINE Matrix3 MakeScale( const Vector3 &scale ) { return Matrix3(XMMatrixScalingFromVector(scale)); }

		INLINE operator XMMATRIX() const { return (const XMMATRIX&)m_mat; }

		INLINE Vector3 operator* ( const Vector3 &vec ) const { return Vector3( XMVector3TransformNormal(vec, *this) ); }
