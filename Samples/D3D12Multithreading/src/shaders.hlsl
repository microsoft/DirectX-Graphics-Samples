...
//--------------------------------------------------------------------------------------
float4 CalcUnshadowedAmountPCF2x2(int lightnum, float4 vPosWorld)
{
	Texture2D txShadow = shadowMap;

	// Compute pixel position in light space
	float4 vLightSpacePos = vPosWorld;
	vLightSpacePos = mul(vLightSpacePos, lights[lightnum].view);
	vLightSpacePos = mul(vLightSpacePos, lights[lightnum].projection);
