...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    UINT backBufferWidth = max(m_outputSize.right - m_outputSize.left, 1l);
    UINT backBufferHeight = max(m_outputSize.bottom - m_outputSize.top, 1l);
...
void DeviceResources::InitializeAdapter(IDXGIAdapter1** ppAdapter)
{
    *ppAdapter = nullptr;
    ...
    for (UINT adapterID = 0; DXGI_ERROR_NOT_FOUND != m_dxgiFactory->EnumAdapters1(adapterID, &adapter); ++adapterID)
    {
        ...
        if (SUCCEEDED(D3D12CreateDevice(adapter.Get(), m_d3dMinFeatureLevel, _uuidof(ID3D12Device5), nullptr)))
        {
