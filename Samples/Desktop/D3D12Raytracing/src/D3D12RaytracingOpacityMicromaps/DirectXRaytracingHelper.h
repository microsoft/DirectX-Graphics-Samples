...
inline void AllocateUploadableBuffer(ID3D12Device* pDevice, void *pData, UINT64 datasize, 
    ID3D12Resource **ppUploadResource, ID3D12Resource **ppDefaultResource, const wchar_t* resourceName = nullptr)
{
    auto uploadHeapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
    ...
    ThrowIfFailed(pDevice->CreateCommittedResource(
        ...
        nullptr,
        IID_PPV_ARGS(ppDefaultResource)));


    if (resourceName)
    {
        (*ppUploadResource)->SetName(resourceName);
        (*ppDefaultResource)->SetName(resourceName);
    }

    if (pData)
    {
        void* pMappedData;
        (*ppUploadResource)->Map(0, nullptr, &pMappedData);
        memcpy(pMappedData, pData, datasize);
        (*ppUploadResource)->Unmap(0, nullptr);
    }
}
...
inline bool IsDirectXRaytracingSupported(IDXGIAdapter1* adapter, D3D12_RAYTRACING_TIER requiredTier)
{
    ComPtr<ID3D12Device> testDevice;
    D3D12_FEATURE_DATA_D3D12_OPTIONS5 featureSupportData = {};

    HRESULT hrDevice = SUCCEEDED(D3D12CreateDevice(adapter, D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&testDevice)));
