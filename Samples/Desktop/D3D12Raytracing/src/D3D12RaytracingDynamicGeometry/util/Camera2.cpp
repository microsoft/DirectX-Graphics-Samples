...
void BaseCamera::SetLookDirection( XMVECTOR forward, XMVECTOR up )
{
    // Given, but ensure normalization
    Scalar forwardLenSq = LengthSquare(forward);
    forward = Select(forward * RecipSqrt(forwardLenSq), -XMVECTOR(kZUnitVector), forwardLenSq < Scalar(0.000001f));

    // Deduce a valid, orthogonal right vector
    XMVECTOR right = Cross(forward, up);
    Scalar rightLenSq = LengthSquare(right);
    right = Select(right * RecipSqrt(rightLenSq), Quaternion(XMVECTOR(kYUnitVector), -XM_PIDIV2) * forward, rightLenSq < Scalar(0.000001f));
...
void BaseCamera::Update()
{
    ...
    m_ViewMatrix = XMMATRIX(~m_CameraToWorld);
