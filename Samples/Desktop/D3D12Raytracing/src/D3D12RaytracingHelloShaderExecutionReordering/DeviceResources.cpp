...
#if defined(_DEBUG)
    //ComPtr<ID3D12DebugDevice1> pDebugDevice;
    //m_d3dDevice->QueryInterface(IID_PPV_ARGS(&pDebugDevice));
    //auto mode = D3D12_DEBUG_DEVICE_BYTECODE_VALIDATION_DISABLED;
    //pDebugDevice->SetDebugParameter(D3D12_DEBUG_DEVICE_PARAMETER_BYTECODE_VALIDATION_MODE, &mode, sizeof(mode));
#endif
