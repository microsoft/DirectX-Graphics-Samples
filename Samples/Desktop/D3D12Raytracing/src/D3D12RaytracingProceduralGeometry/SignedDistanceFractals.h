...
// Retursn a signed distance to a tetrahedron fractal.
// Ref: http://blog.hvidtfeldts.net/index.php/2011/08/distance-estimated-3d-fractals-iii-folding-space/
float sdFractalTetrahedron(float3 position, in UINT Iterations, in float scale = 2.0f)
{
    ...
    while (n < Iterations) 
    {
        // Find the closest tetrahedron vertex to the position.
