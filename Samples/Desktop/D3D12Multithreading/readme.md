...
This sample can be built with with Enhanced Barriers. Make sure to have the latest driver installed and that your GPU supports this feature. To enable it choose `<Config Option> (Enhanced Barriers)`  option under the configuration dropdown. `<Config Option> (Legacy Barriers)` are also available if running on older hardware.