...
#include "CompiledShaders\OpaqueTraverseShaderLib.h"
...
namespace FallbackLayer
{
        void BVHTraversalShaderBuilder::Compile(
            _In_ bool IsAnyHitUsed,
            _Out_ TraversalShader &traversalShader)
    {
            traversalShader = {};
            traversalShader.m_TraversalShaderDxilLib.DXILLibrary.BytecodeLength = IsAnyHitUsed ? sizeof(g_pTraverseShaderLib) : sizeof(g_pOpaqueTraverseShaderLib);
            traversalShader.m_TraversalShaderDxilLib.DXILLibrary.pShaderBytecode = IsAnyHitUsed ? g_pTraverseShaderLib : g_pOpaqueTraverseShaderLib;
#if PROFILE_STATE_MACHINE
